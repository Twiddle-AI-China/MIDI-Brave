...
class _FakeDifferentiableClap(nn.Module):
    """Small deterministic stand-in for the external CLAP package."""
    ...
    def get_audio_embedding_from_data(self, waveforms, use_tensor=True):
        ...
        for waveform in waveforms:
            ...
            features.append(torch.stack((
                waveform.mean(), waveform.square().mean().sqrt(), waveform.abs().mean(),
...
def test_frozen_clap_window_loss_is_aligned_and_differentiable():
    ...
    valid = torch.tensor([2048, 1536])
    identical = objective.waveform_gradients(target, target, valid)
    assert identical.losses.abs().max().item() < 1e-6
    prediction = torch.roll(target, 137, dims=-1) * 0.7
    different = objective.waveform_gradients(prediction, target, valid)
    assert different.losses.mean().item() > 1e-6
...
def test_clap_waveform_gradient_injection_matches_direct_chain_rule():
    ...
    decoder = nn.Conv1d(1, 1, 1, bias=False)
    source = torch.randn(2, 1, 1024)
    target = torch.randn(2, 1, 1024)
    valid = torch.full((2,), 1024, dtype=torch.long)

    direct_prediction = decoder(source)
    direct_embedding = objective._embedding(direct_prediction, valid)
    with torch.no_grad():
        target_embedding = objective._embedding(target, valid)
    direct_loss = (1 - torch.nn.functional.cosine_similarity(
        direct_embedding, target_embedding, dim=-1)).clamp_min(0.0).sum()
    direct_loss.backward()
    direct_gradient = decoder.weight.grad.detach().clone()

    decoder.zero_grad(set_to_none=True)
    injected_prediction = decoder(source)
    waveform = objective.waveform_gradients(injected_prediction, target, valid)
    (injected_prediction * waveform.gradients).sum().backward()
